...
def create_prioritised_item_buffer(
    ...
    def add_fn(
        ...
        flattened_batch = jax.tree.map(lambda x: x.reshape((1, batch_size * seq_len, *x.shape[2:])), batch)
    ...
    def sample_fn(
        ...
        return PrioritisedTrajectoryBufferSample(experience=sampled_batch, indices=indices, probabilities=probabilities)
